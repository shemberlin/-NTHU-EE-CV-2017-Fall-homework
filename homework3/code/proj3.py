...
def main():
    ...
    if FEATURE == 'tiny_image':
        ...
        vocab_size = 500 
        step = [10,10]
        if os.path.isfile('vocab' + '_'+ str(vocab_size) + '_' + str(step[0]) + '.pkl') is False:
            print('No existing visual word vocabulary found. Computing one from training images\n')
#            vocab_size = 400   ### Vocab_size is up to you. Larger values will work better (to a point) but be slower to comput.
            
            vocab = build_vocabulary(train_image_paths, vocab_size, step)
            with open('vocab' + '_'+ str(vocab_size) + '_' + str(step[0]) + '.pkl', 'wb') as handle:
                pickle.dump(vocab, handle, protocol=pickle.HIGHEST_PROTOCOL)

        if os.path.isfile('train_image_feats' + '_' + str(step[0]) + '.pkl') is False:
            # YOU CODE get_bags_of_sifts.py
            train_image_feats = get_bags_of_sifts(train_image_paths,step);
            with open('train_image_feats' + '_' + str(step[0]) + '.pkl', 'wb') as handle:
                pickle.dump(train_image_feats, handle, protocol=pickle.HIGHEST_PROTOCOL)
        else:
            with open('train_image_feats' + '_' + str(step[0]) + '.pkl', 'rb') as handle:
                train_image_feats = pickle.load(handle)

        if os.path.isfile('test_image_feats' + '_' + str(step[0]) + '.pkl') is False:
            test_image_feats  = get_bags_of_sifts(test_image_paths,step);
            with open('test_image_feats' + '_' + str(step[0]) + '.pkl', 'wb') as handle:
                pickle.dump(test_image_feats, handle, protocol=pickle.HIGHEST_PROTOCOL)
        else:
            with open('test_image_feats' + '_' + str(step[0]) + '.pkl', 'rb') as handle:
    ...
    if CLASSIFIER == 'nearest_neighbor':
        ...
        predicted_categories = svm_classify(train_image_feats, train_labels, test_image_feats,C)
    ...
    Re_cm = build_confusion_mtx(test_labels_ids, predicted_categories_ids, ABBR_CATEGORIES)
...
if __name__ == '__main__':
    Re_cm = main()
    sum_cm = np.sum(Re_cm , axis = 1)
    for i in range(len(Re_cm)):
        all_acc.append(Re_cm[i][i]/sum_cm[i])
